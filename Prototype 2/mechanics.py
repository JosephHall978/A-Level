...
def draw_text_mid(surf, text, size, x, y, colour):#takes in surface, text to output, size of text, and x and y coordinates
    font = pygame.font.Font(str(pathlib.Path.cwd().joinpath('SFPixelateShaded.ttf')),size)
...
def draw_text_left(surf, text, size, x, y, colour):#takes in surface, text to output, size of text, and x and y coordinates
    font_name = pygame.font.match_font("sf pixelate shaded")#sets font to comic sans
...
def hearts(surf,lives,height,width):
    heart_img = pygame.image.load(str(pathlib.Path.cwd().joinpath('img','heart-25x25.png')))
    for i in range (lives):
        surf.blit(heart_img,(10+i*30, 10))
def menu(screen, height, width):#creates menu to display on the screen
    ...
    draw_text_mid(screen, "Play", 30,(width//2), (height - 200),yellow)
    draw_text_mid(screen, "Score Board", 30,(width//2), (height - 150),yellow)
    draw_text_mid(screen, "Instructions", 30,(width//2), (height - 100),yellow)
def menu_selector(surf,option,height,width):
    selector_img = pygame.image.load(str(pathlib.Path.cwd().joinpath('img','menu-arrow-25x25.png')))
    if option == 0:
        surf.blit(selector_img,(width//2-60,height-202))
    if option == 1:
        surf.blit(selector_img,(width//2-135,height-150))
    if option == 2:
        surf.blit(selector_img,(width//2-135,height-100))
def high_score(screen, gameStatus, width, height):#creates high score board
    ...
    for v in range(10):#takes first 10 scores
        if gameStatus == "High Score":
            draw_text_left(screen,"Back to menu",15,10,10,yellow)
            draw_text_mid(screen,"Top 10 Score",30,(width//2),30,yellow)
...
def instructions(screen, width,height):
    ...
    draw_text_left(screen, "Use arrow keys or 'W','A','S','D'", 20, width//14, 120,yellow)
    draw_text_left(screen,"Press escape to pause or return to main menu",20,width//14,150,yellow)
    draw_text_left(screen, "Controllers not supported yet", 20, width//14, 180,yellow)
