...
def new_asteroid(asteroid_img):
    a = Asteroid(asteroid_img)
    all_sprites.add(a)
    mobs.add(a)
def new_bomber(bomber_img,fps):
    b = Bomber(bomber_img,fps)
    all_sprites.add(b)
    mobs.add(b)
def new_enemy():
    ...
    if select == 0:
        new_dart(dart_img)
    if select == 1:
        new_asteroid(asteroid_img)
    if select == 2:
        new_bomber(bomber_img,fps)
...
while run:#starts gameplay loop
    ...
    if gameStatus == "Instructions":
        ...
        for event in game_events:
            if event.type == pygame.MOUSEBUTTONUP:
                if (mouse[0]> 10) and (mouse[0] < (100)) and (mouse[1]> 10) and (mouse[1]<35):
                    gameStatus = "Menu"
